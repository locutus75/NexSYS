 ...
 */
export async function claimNevmToUtxoMint(
  rpcClient: SyscoinRpcClient,
  network: NetworkEnvironment,
  ethtxid: string,
  destUtxoAddress: string
): Promise<string> {
  const blockbookUrl = network === "MAINNET" ? MAINNET_BLOCKBOOK : TESTNET_BLOCKBOOK;
  const sysNetwork = network === "MAINNET" ? syscoinUtils.syscoinNetworks.mainnet : syscoinUtils.syscoinNetworks.testnet;
  const syscoinjs = new SyscoinJSLib(null, blockbookUrl, sysNetwork);

  // For NEVM to UTXO, we need the Ethereum SPV proof.
  // syscoinjs-lib can generate this if we provide the ethtxid and a web3url.
  const web3url = network === "MAINNET" 
    ? "https://rpc.syscoin.org" 
    : "https://rpc.tanenbaum.io";

  const assetOpts = {
    ethtxid: ethtxid.startsWith("0x") ? ethtxid.slice(2) : ethtxid,
    web3url: web3url
  };
  ...
  let psbtResult;
  try {
    psbtResult = await syscoinjs.assetAllocationMint(
      assetOpts,
      txOpts,
      null, // assetMap is auto-generated by syscoinjs-lib if null when using eth-proof
    ...
    throw new Error(`Failed to create PSBT for NEVM->UTXO mint: ${err.message}`);
  }
